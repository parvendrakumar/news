...
<div class="flex items-center gap-4 mb-8">
    ...
    <div>
        <h2 class="text-3xl font-black text-slate-800 tracking-tight">Edit Role</h2>
        <p class="text-slate-400 font-bold text-sm">Updating <span class="text-slate-600"><?= esc($role['name']) ?></span></p>
    </div>
</div>
...
<?php $savedPerms = json_decode($role['permissions'] ?? '{}', true) ?: []; ?>
...
<div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
    <form action="<?= base_url('admin/roles/update/' . $role['id']) ?>" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="<?= esc(old('name', $role['name'])) ?>"
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                foreach ($allPerms as $key => $label):
                    $checked = isset($savedPerms[$key]) || isset($savedPerms['all']);
                ?>
