...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-3xl font-black text-slate-800 tracking-tight">Create Flash Alert</h2>
        <p class="text-slate-400 font-bold text-sm">Add an urgent update to the ticker</p>
    </div>
...
<form action="<?= base_url('admin/ticker/store') ?>" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    <?= csrf_field() ?>
        ...
        <div class="bg-white p-4 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="content_hi" rows="3" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-red-500 outline-none font-bold" placeholder="खबर यहाँ लिखें..." required></textarea>
                    <p class="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-tight italic">Recommended length: 80-120 characters</p>
                </div>
                <div>
                    <label class="block text-sm font-black text-slate-700 mb-2">Ticker Message (English)</label>
                    <textarea name="content_en" rows="3" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-red-500 outline-none font-bold" placeholder="Write message in English..."></textarea>
                </div>
                <div>
                    <label class="block text-sm font-black text-slate-700 mb-2">Source Link (Optional)</label>
                    <input type="url" name="link" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-red-500 outline-none font-bold text-blue-600" placeholder="https://news-location.com/...">
                </div>
            </div>
        </div>
    </div>
