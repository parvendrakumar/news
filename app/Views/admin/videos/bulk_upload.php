...
<div class="mb-8">
    <a href="<?= base_url('admin/videos') ?>" class="text-slate-400 font-bold text-sm hover:text-blue-600 transition flex items-center gap-2">
        <i class="fas fa-arrow-left text-xs"></i> BACK TO VIDEO LIBRARY
    </a>
    <h2 class="text-3xl font-black text-slate-800 tracking-tight mt-2">Bulk Video News Upload</h2>
    <p class="text-slate-400 font-bold text-sm">Import multiple video bulletins at once using a CSV file</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-12">
            <form action="<?= base_url('admin/videos/bulk-store') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="<?= base_url('admin/videos/bulk-format') ?>" class="bg-slate-100 text-slate-700 px-8 py-4 rounded-2xl font-black hover:bg-slate-200 transition border border-slate-100">
    ...
    <div class="lg:col-span-1">
        <div class="bg-slate-900 rounded-[2rem] p-8 text-white h-full relative overflow-hidden shadow-2xl">
            <div class="absolute top-0 right-0 p-8 opacity-10">
                <i class="fas fa-video text-8xl"></i>
            </div>
            ...
            <ul class="space-y-6">
                <li class="flex gap-4">
                    ...
                    <div>
                        <p class="font-black text-sm uppercase tracking-wider mb-1">Video URLs</p>
                        <p class="text-slate-400 text-sm font-medium leading-relaxed">Provide valid YouTube, Vimeo, or direct embed URLs in the <strong class="text-white">video_url</strong> column.</p>
                    </div>
                </li>
                <li class="flex gap-4">
                    <div class="mt-1 text-blue-500"><i class="fas fa-check-circle"></i></div>
                    <div>
                        <p class="font-black text-sm uppercase tracking-wider mb-1">Thumbnails</p>
                        <p class="text-slate-400 text-sm font-medium leading-relaxed">Thumbnails must be uploaded manually after bulk import by editing each video bulletin.</p>
                    </div>
                </li>
            </ul>
        </div>
