...
<head>
    ...
    <title>Reset Password - City News</title>
...
<body class="bg-gradient-to-br from-red-600 to-gray-900 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="max-w-md w-full glass rounded-3xl shadow-2xl p-10">
        ...
        <form action="<?= base_url('auth/attempt-reset-password') ?>" method="POST" class="space-y-6">
            <?= csrf_field() ?>
            <input type="hidden" name="token" value="<?= $token ?>">
            
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">New Password</label>
                <input type="password" name="password" class="w-full px-5 py-4 rounded-xl border border-gray-200 focus:border-red-600 focus:ring-4 focus:ring-red-100 outline-none transition" placeholder="Min 8 characters" required autofocus>
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Confirm New Password</label>
                <input type="password" name="confirm_password" class="w-full px-5 py-4 rounded-xl border border-gray-200 focus:border-red-600 focus:ring-4 focus:ring-red-100 outline-none transition" placeholder="Repeat password" required>
            </div>
