...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-3xl font-black text-slate-800 tracking-tight">Edit Category</h2>
        <p class="text-slate-400 font-bold text-sm">Update: <span class="text-red-500"><?= esc($hi['title']) ?></span></p>
    </div>
...
<div class="max-w-4xl">
    <div class="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
        <form action="<?= base_url('admin/categories/update/' . $category['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= csrf_field() ?>
                ...
                <div class="relative group">
                    ...
                    <label for="category_image" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-slate-200 rounded-[2rem] bg-slate-50 cursor-pointer group-hover:bg-slate-100 group-hover:border-red-600 transition-all overflow-hidden sticky-image-label">
                        <?php if (!empty($category['image'])): ?>
                            <img id="image_preview" src="<?= base_url('uploads/categories/' . $category['image']) ?>" class="w-full h-full object-cover">
                            <div id="image_placeholder" class="hidden flex flex-col items-center justify-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title_hi" value="<?= esc($hi['title']) ?>" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-red-600 outline-none font-bold" required>
                </div>
                <div>
                    <label class="block text-sm font-black text-slate-700 mb-2">Title (English)</label>
                    <input type="text" name="title_en" value="<?= esc($en['title'] ?? '') ?>" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-red-600 outline-none font-bold">
                </div>
            </div>

            <div>
                <label class="block text-sm font-black text-slate-700 mb-2">URL Slug</label>
                <input type="text" name="slug" value="<?= esc($category['slug']) ?>" class="w-full px-5 py-3 rounded-xl border border-slate-200 font-bold text-blue-600 bg-blue-50/30" required>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="parent_id" class="w-full px-5 py-3 rounded-xl border border-slate-200 font-bold text-slate-700 appearance-none bg-white">
                        ...
                        <?php foreach ($parents as $p): ?>
                            <option value="<?= $p['id'] ?>" <?= $p['id'] == $category['parent_id'] ? 'selected' : '' ?>><?= $p['title'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="<?= esc($category['sort_order']) ?>" class="w-full px-5 py-3 rounded-xl border border-slate-200 font-bold text-slate-700 bg-white" placeholder="0">
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-slate-50">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="meta_title" value="<?= esc($category['meta_title'] ?? '') ?>" class="w-full px-5 py-3 rounded-xl border border-slate-100 bg-slate-50 font-bold text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Meta Description</label>
                        <textarea name="meta_description" rows="3" class="w-full px-5 py-3 rounded-xl border border-slate-100 bg-slate-50 font-bold text-sm resize-none" placeholder="Describe this section for search engines..."><?= esc($category['meta_description'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>
