...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-3xl font-black text-slate-800 tracking-tight">Create Ad Slot</h2>
        <p class="text-slate-400 font-bold text-sm">Define a new monetization area</p>
    </div>
...
<form action="<?= base_url('admin/ads/store') ?>" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    <?= csrf_field() ?>
        ...
        <div class="bg-white p-4 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="slot_name" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-green-500 outline-none font-bold uppercase tracking-wide" placeholder="E.G. HOMEPAGE_TOP_BANNER" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="target_page" id="target_page" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-green-500 outline-none font-bold text-slate-600 appearance-none bg-slate-50" onchange="toggleCategorySelect()">
                            ...
                            <option value="news_detail">News Detail Pages Only</option>
                        </select>
                    </div>

                    <div id="category_select_container" class="hidden">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="ad_type" value="image" class="peer sr-only" checked onchange="toggleAdType('image')">
                        ...
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="ad_type" value="google_ads" class="peer sr-only" onchange="toggleAdType('google')">
                        ...
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="ad_type" value="custom_code" class="peer sr-only" onchange="toggleAdType('custom')">
                ...
                <div id="image-fields" class="space-y-6">
                    <div>
                        ...
                        <div class="relative group cursor-pointer h-40 rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center bg-slate-50 hover:bg-green-50 hover:border-green-200 transition">
                            ...
                            <div id="image-placeholder" class="text-center">
                                ...
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Upload Advertisement File</p>
                            </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="link" class="w-full px-5 py-4 rounded-2xl border border-slate-200 focus:border-green-500 outline-none font-bold text-slate-600" placeholder="https://external-link.com/...">
                    </div>
                </div>

                <div id="code-fields" class="hidden">
                    <label class="block text-sm font-black text-slate-700 mb-2">External Code / JS Snippet</label>
                    <textarea name="custom_code" rows="10" class="w-full px-5 py-4 rounded-3xl border border-slate-200 focus:border-indigo-500 outline-none font-mono text-xs bg-slate-900 text-green-400" placeholder="<script>...</script>"></textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-4 space-y-6">
        <div class="bg-white p-4 md:p-8 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div class="font-black text-xs text-slate-600 uppercase tracking-widest">Activate Slot</div>
        ...
        <div class="bg-slate-900 p-8 rounded-[2rem] shadow-2xl text-white relative overflow-hidden">
            ...
            <h3 class="text-xs font-black text-green-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                <i class="fas fa-ruler-combined"></i> Banner Dimensions
            </h3>
            
            <div class="space-y-5">
                <div class="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Header / Billboard</div>
                    <div class="text-sm font-bold text-white">728 x 90 <span class="text-slate-500 font-medium">px</span></div>
                </div>
                <div class="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Sidebar Rectangle</div>
                    <div class="text-sm font-bold text-white">300 x 250 <span class="text-slate-500 font-medium">px</span></div>
                </div>
                <div class="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Sidebar Skyscraper</div>
                    <div class="text-sm font-bold text-white">300 x 600 <span class="text-slate-500 font-medium">px</span></div>
                </div>
                <div class="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">In-Article Content</div>
                    <div class="text-sm font-bold text-white">640 x 100 <span class="text-slate-500 font-medium">px</span></div>
                </div>
            </div>

            <div class="mt-6 p-4 bg-green-500/10 rounded-2xl border border-green-500/20 text-[10px] font-medium text-green-300 leading-relaxed">
                <i class="fas fa-info-circle mr-1"></i> Use these standard IAB sizes to ensure the best visibility and CTR across devices.
            </div>
        </div>
    </div>
</form>
